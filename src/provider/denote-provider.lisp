...
(defmethod provide-all ((prov denote-provider) &rest script-args)
  (let* ((files (apply #'call-next-method prov script-args))
         (notes (mapcar (op (from _ 'note)) files)))

    (dolist (note notes)
      (setf (@ (denote-store prov) (note-id note)) note))

    (dolist (note notes)
      (setf (note-linked-notes note)
            (provide-linked-denotes prov :note note)))

    notes))

(defmethod provide-linked-denotes ((prov denote-provider) &key note)
...
far. STORE is needed to resolve circular dependencies of denotes."
  (let* ((linked-denote-ids (concatenate 'list (lquery:$ (inline (note-body-dom note))
                                                 "a[data-denote-id]"
                                                 (attr "data-denote-id"))))
    ...
    (unless (emptyp missing-linked-ids)
      (provide-all prov :ids linked-denote-ids))

    (mapcar (op (@ (denote-store prov) _))
            linked-denote-ids)))
